...
model_binom %>%
  ...
  kable(caption = "Parameter estimates from logistic regression.", align = "rr")
```

```{r}
model_binom %>% glance %>% round(2)
```
...
glm_coef(model_pois) %>% kable(align = "rr")
```
...
```{r}
plot_model(model_lme, type = "pred", terms = age ~ Sex, show.data = TRUE) %>%
