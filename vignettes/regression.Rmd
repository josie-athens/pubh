...
A linear model with mixed effects can be used to control for correlations between matched cases, as an alternative to conditional logistic. Discussions about correlation structures are beyond the scope of the vignette.

```{r}
require(lme4)
model_match <- glmer(cancer ~ est * gall + 
                       (1|set), data=bdendo, family=binomial)
```
...
```{r}
model_match %>% glance %>% round(digits = 2)
```


```{r}
plot_model(model_match, type = 'pred', terms = c("est", "gall"), dot.size = 2)
```

```{r}
emmip(model_match, gall ~ est, type = "response") %>%
  gf_labs(x="Oestrogen", y = "P (cancer)", col = get_label(bdendo$gall))
```
...
quine %>%
  ...
  summarise(
    n = n(),
    Mean = round(w_mean(Days), 2),
    SD = round(w_sd(Days), 2),
    Median = round(w_median(Days), 2)
  ) %>%
  kable()
```
...
```{r}
multiple(model_negbin, ~ Age + Sex|Eth)$fig_pval %>%
  gf_labs(y = "Age group")
```
