...
We can use `plot_model` from the `sjPlot` package to construct the effect plot. The advantage of `plot_model` is that it recognises labelled data and uses that information to annotate the plots.
...
Another option is to use functions from `easystats`, `estimate_means` calculates the mean birth weight for each combination with corresponding confidence intervals and `see` plots them.
