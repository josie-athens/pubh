...
birthwt %>%
  tab_cells(bwt) %>%
  tab_rows(race, smoke) %>%
  tab_stat_fun("Mean" = w_mean, "Median" = w_median, "SD"=w_sd, 
               "CV"=rel_dis, method = list) %>%
  tab_pivot()
```
...
emmip(model_norm, ~ smoke | race, CIs = T) %>%
  gf_labs(y = get_label(birthwt$bwt))
```
...
```{r}
 bdendo_pred <- predict(model_clogit, bdendo_grid, type = "lp", se.fit =
TRUE)
...
```{r}
bdendo_grid %>%
  gf_pointrange(fit + lo + up ~ gall|est, col = ~ gall) %>%
...
```{r}
emmip(model_negbin, Age ~ Eth|Sex) %>%
  gf_labs(y = "Number of absent days", x = "Ethnicity")
```
