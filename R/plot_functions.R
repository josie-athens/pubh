...
#'
#' @param A ggplot2 object (see examples).
...
#'
#' rvf_plot(model)
rvf_plot <- function(model, pch = 20, col = 'black', xlab = NULL)
{
  .Deprecated("mosaic::mplot")
  xl <- ifelse(is.null(xlab), "Fitted values", xlab)
  fortify(model) %>%
    gf_point(.stdresid ~ .fitted, data = df, pch = pch, col = col) %>%
    gf_smooth(col = 'indianred3', se = FALSE, lwd = 0.5, method = 'loess') %>%
