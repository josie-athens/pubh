...
#' Using labels from variables for axis-labels in plots.
#'
#' \code{use_labels} takes labels from labelled data to use them as axis-labels for plots generated by \code{gformula} or \code{ggplot2}.
#'
#' @param object A ggplot2 object (see examples).
...
use_labels <- function(object)
{
  ...
  outcome <- dt[[y]]
  exposure <- dt[[x]]
  yl <- ifelse(is.null(get_label(outcome)), y, get_label(outcome))
  xl <- ifelse(is.null(get_label(exposure)), x, get_label(exposure))
  object %>%
    gf_labs(x = xl, y = yl)
}
...
#'
#' \code{gf_stars} Is a function used to display differences between groups (see details).
#'
...
#' @seealso \link{emmeans::emmeans}, \link{emmeans::plot.emmGrid}, \link{emmeans::pwpp}
#' @examples
...
#' @param ... Further arguments passed to \link{gf_point}.
#' @examples
...
#'  gf_theme(theme = theme_sjplot2(base_size=9))
#'
#' data(Sharples)
#'
#' Sharples %>%
                         ...
                         pch = 20, size = 1, col = 'black', transform = FALSE, ...)
{
  ...
  if(transform == TRUE) {
    gf_point(Ratio ~ Average, data = df, pch = pch,
             size = size, col = col, ...) %>%
      gf_hline(yintercept = ~ mean(Ratio), col = 2) %>%
      gf_hline(yintercept = ~ lw2, col = 2, lty = 2) %>%
      gf_hline(yintercept = ~ up2, col = 2, lty = 2)
  } else {
    gf_point(Difference ~ Average, data = df, pch = pch,
             size = size, col = col, ...) %>%
      gf_hline(yintercept = ~ mean(Difference), col = 2) %>%
      gf_hline(yintercept = ~ lw1, col = 2, lty = 2) %>%
      gf_hline(yintercept = ~ up1, col = 2, lty = 2)
  }
}

#' Residual vs Fitted plot.
#'
#' \code{rvf_plot} plots studentized residuals against fitted values from \link{glm} objects using package \code{gformula}.
#' @param model A \link{glm} or \link{lm} object with a numerical outcome.
#' @param pch A numeric value passed to \link{gf_point} for the symbol.
#' @param col Colour passed to \link{gf_point}.
#' @param xlab A character passed to \link{gf_point} for the x-axis label.
#' @seealso \link{mosaic::mplot}, \link{GGally::ggally_nostic_std_resid}
#' @examples
#' data(thuesen, package = "ISwR")
#' thuesen <- thuesen %>%
#'   var_labels(
#'     short.velocity = "Circumferential shortening velocity (%/s)",
#'     blood.glucose = "Fasting blood glucose (mmol/l)"
#'   )
#'
#' model <- lm(short.velocity ~ blood.glucose, data = thuesen)
#' mplot(model_2, which=1)[[1]]
#'
#' rvf_plot(model)
#'
#' rvf_plot(model_2, xlab = 'Fitted values')
rvf_plot <- function(model, pch = 20, col = 1, xlab = NULL)
{
  .Deprecated("GGally::ggally_nostic_std_resid")
  xl <- ifelse(is.null(xlab), get_dv_labels(model), xlab)
  df <- fortify(model)
  gf_point(.stdresid ~ .fitted, data = df, pch = pch, col = col) %>%
    gf_smooth(col = 'red', se = FALSE, lwd = 0.5) %>%
    gf_hline(yintercept = ~ 0, col = "gray", lty = 2) %>%
    gf_hline(yintercept = ~ 2, col = "gray", lty = 2) %>%
    gf_hline(yintercept = ~ -2, col = "gray", lty = 2) %>%
    gf_labs(y = 'Standardised residuals', x = xl)
}
...
#' @param data A data frame where the variables in the \code{formula} can be found.
#' @param pch Point character passed to \link{gf_point} or \link{gf_jitter}.
...
strip_error <- function(object = NULL, formula = NULL, data = NULL,
                        pch = 20, size = 0.6, alpha = 0.7, col = 'dodgerblue')
{
  if (inherits(object, "formula")) {
    formula <- object
    object <- NULL
                       ...
                       fill = 'darkgrey', col = 'black', alpha = 0.7)
{
  ...
  if(is.character(fill) & is.character(col)) {
    ...
    gf_summary(formula, data = data, fill = fill, fun.y = "mean", geom = 'bar',
               alpha = alpha, col = col, position = position_dodge()) %>%
      gf_summary(fun.data = "mean_cl_boot", geom = 'errorbar', width = 0.3,
...
#'
#' \code{coef_plot} Constructs a plot displaying estimates of parameters with bars representing confidence intervals.
#' @details \code{coef_plot} does not show estimate for the constant (intercept). Estimates and confidence intervals
#' can be optionally exponentiated for family == "gaussian", in which case estimates  represent ratios instead of differences.
#' Models derived from families different than "gaussian" are automatically exponentiated.
...
#' @param Exp Logical, should estimates and confidence intervals be exponentiated for models with family == "gaussian"?
#' @param subitle Logical, should a subtitle with the name of the outcome be included?
#' @param pch Point character passed to \link{gf_point}.
#' @param ... Further arguments passed to \link{gf_point}.
#' @seealso \link{sjPlot::plot_model}, \link{mosaic::mplot}, \link{GGally::ggcoef}.
#' @examples
#' data(birthwt, package = "MASS")
#' birthwt <- mutate(birthwt,
#'   smoke = factor(smoke, labels = c("Non-smoker", "Smoker")),
...
#'   )
#'
#' model_1 <- glm(bwt ~ smoke + Race, data = birthwt)
#' model_1 %>%
#'   glm_coef(labels=c("Constant", "Smoker vs Non-smoker", "Non-white vs White"))
#'
#' model_1 %>%
#'   coef_plot(labels = c('Non-withe vs White', 'Smoker vs Non-Smoker'))
#'
...
#'   )
#'
#' model_2 <- glm(mf ~ (area + agegrp + sex)^2 - area:agegrp,
#'   data = Oncho, binomial)
#'
#' model_2 %>%
#'   coef_plot()
...
#' model_2 %>%
#'   plot_model()
coef_plot <- function(model, labels = NULL, Exp = FALSE, subtitle = TRUE, pch = 20, ...)
{
  .Deprecated("sjPlot::plot_model")
  subt <- ifelse(isTRUE(subtitle), get_dv_labels(model)[[1]], '')
  if (family(model)$family == 'gaussian' & Exp == FALSE) {
    if (is.null(labels)) {
    ...
    param <- as.factor(row.names(cis))
    cis2 <- data.frame(Parameter = param,
                       Estimate = cis[, 1],
                       lo = cis[, 2],
                       up = cis[, 3])
    gf_point(Parameter ~ Estimate, data = cis2, pch = pch) %>%
      gf_errorbarh(Parameter ~ lo + up, data = cis2, width = 0.3) %>%
      gf_vline(color='red', xintercept= ~ 0, lty = 2) %>%
      gf_labs(x = 'Difference', y = '', subtitle = subt)
  }
  else {
    if (is.null(labels)) {
      cis <- glm_coef(model, type = "ext", exp_norm = Exp)[-1, c(1, 3:4)]
    }
    else {
      cis <- glm_coef(model, labels = c("", labels), type = "ext", exp_norm = Exp)[-1, c(1, 3:4)]
    }
    param <- row.names(cis)
    cis2 <- data.frame(Parameter = param,
                       Estimate = cis[, 1],
                       lo = cis[, 2],
                       up = cis[, 3])
    xl <- ifelse(family(model)$family == 'binomial', 'Odds Ratio', 'Ratio')
    gf_point(Parameter ~ Estimate, data = cis2, pch = pch) %>%
      gf_errorbarh(Parameter ~ lo + up, data = cis2, width = 0.3) %>%
      gf_vline(color='red', xintercept= ~ 1, lty = 2) %>%
      gf_labs(x = xl, y = '', subtitle = subt)
  }
}
...
gen_bst_df <- function (object = NULL, formula = NULL, data = NULL, stat = "mean", ...)
{
  ...
  if (nv == 2) {
    ...
    resp <- ifelse(is.null(get_label(outcome)), resp, get_label(outcome))
    expl <- ifelse(is.null(get_label(exposure)), expl, get_label(exposure))
    names(final) [1] <- resp
    names(final)[4] <- expl
    final
  }
    ...
    resp <- ifelse(is.null(get_label(outcome)), resp, get_label(outcome))
    expl <- ifelse(is.null(get_label(exposure)), expl, get_label(exposure))
    strat <- ifelse(is.null(get_label(strat2)), expl, get_label(strat2))
    names(final) [1] <- resp
    names(final)[4] <- expl
    names(final)[5] <- strat
    final
